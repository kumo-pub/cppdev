...
### worker线程数 {#number-of-worker-threads}
...
### 限制最大并发 {#limiting-maximum-concurrency}
...
krpc同一个进程中所有的server[共用线程](#number-of-worker-threads)，如果创建了多个server，最终的工作线程数多半是最大的那个ServerOptions.num_threads。
